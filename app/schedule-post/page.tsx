          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
            ...
            {!loading && dates.map((date) => (
              <button
                ...
                className="group text-left rounded-2xl border border-white/10 bg-gradient-to-br from-[#0e0f14] to-[#0b0c10] hover:from-[#0f1220] hover:to-[#0d101a] transition-colors shadow-sm overflow-hidden"
              >
                <div className="relative">
        ...
        {selectedDate && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              ...
              <button onClick={() => setSelectedDate(null)} className="px-4 py-2 rounded-lg bg-panel border border-border text-foreground hover:bg-panelHover">Back</button>
                <button onClick={() => scrollTo({top:0, behavior:'smooth'})} className="px-4 py-2 rounded-lg bg-panelActive border border-[#3a3a3a] text-white hover:bg-[#2a2a2a]">Top</button>
              </div>
            </div>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-gradient-to-b from-[#0e0f14] to-[#0b0c10] overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,.35)]">
      <div className="relative aspect-video bg-black">
        ...
        />
        <div className="absolute right-2 top-2 px-2 py-1 rounded bg-black/60 text-[10px] text-white border border-white/10">
          {elapsed}s
        </div>
        {scheduledAt && (
          <div className="absolute left-2 top-2 px-2 py-1 rounded bg-emerald-500/15 text-emerald-200 text-[11px] border border-emerald-500/30">
      ...
      <div className="p-3 border-t border-white/10">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            <ActionDelete path={path} />
            <ActionCaption path={path} setHasCaption={setHasCaption} />
            <ActionViewCaption path={path} hasCaption={hasCaption} />
            <ActionPublish path={path} />
            <button
              title="Schedule"
              aria-label="Schedule"
              className="p-2 rounded-full border border-transparent bg-gradient-to-br from-sky-500/20 via-cyan-400/20 to-indigo-500/20 text-white hover:from-sky-500/30 hover:to-indigo-500/30 shadow-[0_0_0_1px_rgba(99,102,241,.25)]"
...
function ActionDelete({ path }: { path: string }) {
  return (
        <button
          title="Delete clip"
          aria-label="Delete clip"
      className="p-2 rounded-full border border-transparent bg-gradient-to-br from-rose-500/20 via-red-500/15 to-orange-500/15 text-white hover:from-rose-500/30 hover:to-orange-500/25 shadow-[0_0_0_1px_rgba(239,68,68,.25)]"
          onClick={async () => {
...
function ActionCaption({ path, setHasCaption }: { path: string, setHasCaption: (v:boolean)=>void }) {
  return (
        <button
          title="Generate caption"
          aria-label="Generate caption"
      className="p-2 rounded-full border border-transparent bg-gradient-to-br from-fuchsia-500/20 via-violet-500/20 to-indigo-500/20 text-white hover:from-fuchsia-500/30 hover:to-indigo-500/30 shadow-[0_0_0_1px_rgba(167,139,250,.25)]"
          onClick={async () => {
            try {
              ...
              const payload = { title: data.title, caption: data.caption, hashtags: data.hashtags }
              try {
                const key = `caption:${path}`
...
function ActionViewCaption({ path, hasCaption }: { path: string, hasCaption: boolean }) {
  return (
        <button
          title="View caption"
          aria-label="View caption"
      className={`relative p-2 rounded-full border border-transparent ${hasCaption ? 'bg-gradient-to-br from-emerald-500/20 via-teal-500/20 to-green-500/20' : 'bg-gradient-to-br from-emerald-500/10 via-teal-500/10 to-green-500/10'} text-white hover:from-emerald-500/30 hover:to-green-500/30 shadow-[0_0_0_1px_rgba(16,185,129,.25)]`}
          onClick={() => {
            const key = `caption:${path}`
            const raw = localStorage.getItem(key)
...
function ActionPublish({ path }: { path: string }) {
  return (
        <button
          title="Publish to YouTube"
          aria-label="Publish to YouTube"
      className="p-2 rounded-full border border-transparent bg-gradient-to-br from-rose-500/20 via-pink-500/20 to-red-500/20 text-white hover:from-pink-500/30 hover:to-red-500/30 shadow-[0_0_0_1px_rgba(244,63,94,.25)]"
          onClick={async () => {
            const key = `caption:${path}`
            const raw = localStorage.getItem(key)
        ...
        } catch (e) { alert('YouTube publishing failed: ' + e) }
          }}
        >
      <span className="grid place-items-center w-4 h-4">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor" role="img">
